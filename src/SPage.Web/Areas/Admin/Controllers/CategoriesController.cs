...
using SPage.Application.Features.Categories.Commands;
...
public sealed class CategoriesController : Controller
{
    ...
    public async Task<IActionResult> Create(string name, string? description, int? parentId)
    {
        ...
        await _mediator.Send(new CreateCategoryCommand(name, description, parentId));
        TempData["Success"] = "Chuyên mục đã được thêm.";
        return RedirectToAction(nameof(Index));
    }
    ...
    public async Task<IActionResult> Edit(int id, CategoryEditModel model)
    {
        ...
        var success = await _mediator.Send(new UpdateCategoryCommand(
            model.Id,
            model.Name,
            model.Slug,
            model.Description,
