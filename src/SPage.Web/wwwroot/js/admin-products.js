document.addEventListener('DOMContentLoaded', () => {
    initProductSlug();
    initProductMetaCounters();
    initPrimaryImageUpload();
    initGalleryUpload();
    initProductEditor();
});
...
function initProductMetaCounters() {
...
async function uploadImageFromInput(fileInputId, targetInputId, previewId = null) {
    ...
    if (!uploadUrl) {
        console.error('Upload URL is missing for', fileInputId);
        return;
    }
...
function initPrimaryImageUpload() {
    const fileInput = document.getElementById('primary-image-file');
    attachPreviewListener('primary-image-url', 'primary-image-preview');

    // Auto-fill Alt Text if empty
    const altInput = document.getElementById('PrimaryImageAlt');
    ...
    fileInput.addEventListener('change', () => {
        uploadImageFromInput('primary-image-file', 'primary-image-url', 'primary-image-preview');
        updateAltText();
    });
}
...
function initGalleryUpload() {
    ...
    // Auto-fill Alt Text if empty for gallery
    const altInput = document.querySelector('input[name="altText"]');
    const titleInput = document.getElementById('Name') || document.getElementById('Title');
    ...
    fileInput.addEventListener('change', () => {
        uploadImageFromInput('gallery-image-file', 'gallery-image-url');
        updateAltText();
    });
}

function initProductEditor() {
    const editorEl = document.querySelector('#product-description');
